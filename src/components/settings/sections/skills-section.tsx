...
function frontmatterField(content: string, field: string): string | undefined {
  ...
  const value = new RegExp(`^${field}:\\s*(.+)$`, "m").exec(block)?.[1]?.trim()
  return value?.replace(/^['"]|['"]$/g, "") || undefined
}

async function scanProjectSkills(projectPath: string): Promise<AvailableAgentSkill[]> {
  const root = `${projectPath.replace(/\\/g, "/").replace(/\/+$/, "")}/.llm-wiki/skills`
  const files = flattenFiles(await listDirectory(root, true))
    .filter((file) => file.name === "SKILL.md")
  return Promise.all(files.map(async (file) => {
    const content = await readFile(file.path)
    const parent = file.path.replace(/\\/g, "/").split("/").slice(-2, -1)[0] || "skill"
    return {
      id: parent,
      name: frontmatterField(content, "name") ?? parent,
...
export function SkillsSection() {
  ...
  const scan = useCallback(async () => {
    ...
    setStatus(null)
    try {
      const found = await scanProjectSkills(project.path)
      setSkills(found)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {t("settings.sections.skills.description", {
            defaultValue:
              "Scan this project's skill folder, then choose which skills can be used from Chat.",
          })}
        </p>
      </div>
      ...
      <div className="rounded-md border bg-muted/20 p-3 text-xs text-muted-foreground">
        {t("settings.sections.skills.paths", {
          defaultValue:
            "Scanned folder: .llm-wiki/skills.",
        })}
      </div>
