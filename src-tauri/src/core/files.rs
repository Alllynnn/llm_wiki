//! Pure file-IO logic — no Tauri, no AppHandle.
//!
//! Covers: binary file read/write, base64 helpers, copy/delete/exists/metadata,
//! MD5, and the global PDFium handle (used by both this module and
//! `core::extract::pdf`).
//!
//! All 11 public functions are called by thin `#[tauri::command]` wrappers in
//! `commands::fs`. Adding HTTP handlers in Phase 4 should only require calling
...
// ──────────────────────────────────────────────────────────────────────────
// Global PDFium handle + serialization lock
// ──────────────────────────────────────────────────────────────────────────
...
pub(crate) fn require_absolute_path(operation: &str, path: &str) -> Result<(), String> {
...
pub(crate) fn is_absolute_path_cross_platform(path: &str) -> bool {
    if path.is_empty() {
        return false;
    }
...
pub(crate) fn extract_office_text(path: &str, ext: &str) -> Result<String, String> {
...
/// Read a file, performing text extraction for PDF/Office/image/media formats.
...
/// Delete a file or directory (with retry on Windows transient errors).
...
/// Read any file as base64 + a guessed mime type.
...
mod tests {
    ...
    /// Smoke test: a real PDF panic (synthesized) is caught.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
