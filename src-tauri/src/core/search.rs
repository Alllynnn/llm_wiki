//! Pure hybrid BM25+vector search logic — no Tauri, no AppHandle.
//!
//! The single public entry point is `search_project`. It is called by the
//! thin `#[tauri::command]` wrapper in `commands::search`, and will be called
//! directly by the axum HTTP handler in Phase 4.
...
pub enum SearchError {
    ...
    Vectorstore(#[from] crate::core::vectorstore::VectorstoreError),
    #[error("io error: {0}")]
...
async fn search_by_embedding(
    ...
    let raw_chunks = crate::core::vectorstore::vector_search_chunks(
    ...
    let mut by_page: BTreeMap<String, Vec<crate::core::vectorstore::ChunkSearchResult>> =
        BTreeMap::new();
